...
* atom:string
  ...
  * string ('"' preserved)
  * other (typically '#...")
  (i.e. all atoms are provided as strings, even identifiers (would be symbol))
...
qsptr_t qssexpr_revlist_to_qsutf8 (qsheap_t * mem, qsptr_t revlist)
{
  ...
  return qsstr_inject(mem, payload, lislen);
}
...
int qssxparse_v0_feed (qsheap_t * mem, struct qssxparse_s * parser, int ch, qsptr_t * out)
{
  ...
  while (pending)
	...
	case PARSER_APPEND_ATOM:
	  ...
	  atomval = qssexpr_revlist_to_qsutf8(mem, parser->lexeme);
