...
/*
Command Console S-Expression reader, "loose reader".
...
initial '(' starts list, parsing closes with '\n' (implied list ending)
initial '/' starts list, list ends with '\n' (for console '/' commands)
initial '+' starts implicit set! list (for argv '+' options)
initial '!' starts normal S-Expression (chat-escape for console)
*/
...
static
struct matchrule_s matchrule2_INIT[] = {
      { is_end, READER_END, PARSER_DISCARD },
      { is_whitespace, READER_WS, PARSER_DISCARD },
      { is_op, READER_WS, PARSER_BEGIN_LIST },
      { is_cl, READER_WS, PARSER_END_LIST },
      { is_string_delimit, READER_STRING, PARSER_CONSUME },
      { is_console_xlist, READER_WS, PARSER_BEGIN_LIST },
      { is_console_sxp, READER_WS, PARSER_DISCARD },
      { is_any, READER_ATOM, PARSER_CONSUME },
      { NULL, 0, 0 },
};

static
struct matchrule_s matchrule2_WS[] = {
      { is_end, READER_END, PARSER_DISCARD },
      { is_whitespace, READER_WS, PARSER_DISCARD },
      { is_op, READER_WS, PARSER_BEGIN_LIST },
      { is_cl, READER_WS, PARSER_END_LIST },
...
static
struct matchrule_s matchrule2_CHAT[] = {
      { is_end, READER_END, PARSER_DISCARD },
      { is_any, READER_CHAT, PARSER_CONSUME },
      { NULL, 0, 0 },
};

static
struct matchrule_s matchrule2_ATOM[] = {
      { is_end, READER_END, PARSER_APPEND_ATOM },
      { is_whitespace, READER_WS, PARSER_APPEND_ATOM },
      { is_op, READER_WS, PARSER_APPEND_ATOM },
      { is_cl, READER_WS, PARSER_APPEND_ATOM },
...
static
struct matchrule_s matchrule2_STRING[] = {
...
static
struct matchrule_s matchrule2_STRING_ESCAPE[] = {
      { is_end, READER_END, PARSER_FORCE_END_STRING },
      { is_any, READER_STRING, PARSER_CONSUME },
      { NULL, 0, 0 },
};
...
static
struct matchrule_s matchrule2_STRING_END[] = {
      { is_end, READER_END, PARSER_APPEND_ATOM },
      { is_any, READER_WS, PARSER_APPEND_ATOM },
      { NULL, 0, 0 },
};

static
struct matchrule_s matchrule2_END[] = {
      { is_any, READER_END, PARSER_HALT },
      { NULL, 0, 0 },
};

static
struct matchrule_s *matchruleset2[READER_MAX] = {
    ...
    matchrule2_ATOM,
    matchrule2_STRING,
    matchrule2_STRING_ESCAPE,
    matchrule2_STRING_END,
    matchrule2_END,
    matchrule2_END,
};
...
*/
int qssxparse_v2_feed (qsheap_t * mem, struct qssxparse_s * parser, int ch, qsptr_t * out)
{
  ...
  while (pending)
      ...
      if (parser->prevch)
	{
      ...
      const struct matchrule_s *matchrules = matchruleset2[parser->state];
      ...
      for (entry = matchrules; entry->predicate; entry++)
	{
      ...
      if ((ch == 0) && !parser->complete)
	{
	  output = PARSER_END_LIST;
	}
